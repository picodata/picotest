...
- [Test framework for Picodata plugins](#test-framework-for-picodata-plugins)
  ...
  - [Совместимость с Pike](#совместимость-с-pike)
...
## Совместимость с Pike

Таблица внизу отражает совместимость **Picotest**'a с плагинами, в которых установлена конкретная версия **Pike**'a.

Например, **Picotest**, начиная с релиза **3.0.0** работает с **Pike**'ом старше **5.0.0**.
...
```bash
RUST_LOG=debug,pike=debug,picotest_helpers=debug cargo test -- --nocapture

RUST_LOG=pike=debug cargo test -- --nocapture
```

### Файлы логов кластера

Pike дублирует вывод инстансов в файлы на диске (независимо от `RUST_LOG`):

```text
{корень_плагина}/{data_dir}/cluster/{имя_инстанса}/picodata.log
```
